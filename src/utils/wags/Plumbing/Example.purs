...
wag { value } = case value of
  Nil -> make 1.0 { earth: s $ " ", preload }
  ({ value: v } : _) -> case v of
    DE'Music_was_never_meant_to_be_static_or_fixed -> make 1.0 { earth: s $ "bassdm", preload }
    DE'Music_must_explode_with_possibilities -> make 1.0 { earth: s $ "tabla ", preload }
    DE'The_possibility_to_add_new_sounds _ -> make 1.0 { earth: s $ "hh ", preload }
